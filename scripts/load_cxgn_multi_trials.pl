...
foreach my $trial_name (@metadata_rows) { 
    $design_type    = $trial_metadata->value_at($trial_name, "design") || 'RCBD' ;
    $year           = $trial_metadata->value_at($trial_name, "year");
    $trial_type     = $trial_metadata->value_at($trial_name, "trial_type");
    $trial_location = $trial_metadata->value_at($trial_name, "trial_location");
...
foreach my $plot_name (@trial_rows) { 
    ...
    foreach my $trait_string ( @traits ) {
...
my $coderef= sub  {
    ...
    print("The trial type is $trial_type \n");
	my $trial_create = CXGN::Trial::TrialCreate->new({
        ...
        has_trial_type    => $multi_trial_data{$trial_name}->{trial_type},
 ...
 #    print("This is the parsers ". Dumper(\%parsed_data)."\n");
	...
	#Safter storing the trial desgin store the phenotypes 
