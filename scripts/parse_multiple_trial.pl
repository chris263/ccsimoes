...
while ($z <= scalar(@list_files)){
	...
	open $fh3, "<", $list_files[$z] or die "Can't open the file '$list_files[$z]' $!\n";
	my $i =1;
	while (my $row = $csv->getline ($fh3)) {
		if ($z == 0 && $i == 1){
			...
			# splice @$row, 9, $size;
			if ($trial_design[$match] eq "Completely Randomized"){
				$trial_design_fixed = 'CRD';
			} elsif ($trial_design[$match] eq "Complete Block") {
				$trial_design_fixed = 'RCBD';
			} else {
				$trial_design_fixed = $trial_design[$match];
			}
			splice @$row, 1,0, $breeding_program[$match],$location[$match], $study_year[$match],$trial_design_fixed,$trial_description[$match],$trial_type[$match],"","","",$planting_date[$match],$harvest_date[$match];
			splice @$row, 18, 0, "";
			push @rows, $row;
		}
		$i++;
...
# and write as CSV
open $fh3, ">", '/Users/christiano/Documents/shared/CIP_Data/multiple_ghana.csv' or die "Can't save the file 'multiple_ghana' $!\n";
print "The new file is 'multiple_ghana'\n";
# print $fh3 (join("\t", @$_), "\n") for @rows;
