...
=head1 SYNOPSIS
    ...
    $load_stock_data.pl -H [dbhost] -D [dbname] -i [infile] [-t]
...
my $population_name = 'Sweetpotato accessions';

my $species = 'Ipomoea batatas';
...
my $coderef= sub  {
	foreach my $num (@rows ) {
		...
		if (!$accession) { next; }
		print "\nSweet potato accession name is '" . $accession . "'\n";
		$count++;
        ...
        my $syn1 =  $spreadsheet->value_at($num, "UNIQUENAME"); #accession uniquename
        my $syn2 =  $spreadsheet->value_at($num, "GENOTYPE");
        my $syn3 =  $spreadsheet->value_at($num, "SYNONYM1");
        my $syn4 =  $spreadsheet->value_at($num, "SYNONYM2");
        ...
        my $notes = $spreadsheet->value_at($num, "NOTES");

        #my $project_description = "$name $project_type ($year) $location";

        print"project descr is $project_description \n";
...
# select * from stock join nd_experiment_stock ON nd_experiment_stock.nd_experiment_stock_id = stock.stock_id join nd_experiment on nd_experiment.nd_experiment_id = nd_experiment_stock.nd_experiment_id join nd_experiment_project on nd_experiment_project.nd_experiment_id = nd_experiment_stock.nd_experiment_id ;
        # my $sp_project = $schema->resultset("Project::Project")->find_or_create(
        # {
     ...
     #    my $geo_description = $location;
		...
		if ($notes) { $stock->create_stockprops( { notes => $notes }, { autocreate=>1})};
        ...
        my @synonyms = ($syn1,$syn2, $syn3, $syn4);
