...
foreach my $name (@files){
	if ($name ne "."){
		if ($name ne ".."){
			...
			my $new_name = $new_folder."pheno_".$check_name.".csv";
...
while ($z <= scalar(@trial_name)){
	...
	while ($i<=$row_max) {
		if($i==0){
			...
			my $trial_name = $worksheet->get_cell($i,0)->value();
			my $plot_name = $worksheet->get_cell($i,1)->value();
